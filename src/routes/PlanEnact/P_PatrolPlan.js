...
import styles from './P_PatrolPlan.less';
...
export default class P_PlanInfo extends Component {
  ...
  // 插入计划
  insertPlan = () => {
    this.props.dispatch(routerRedux.push({pathname: '/query/patrol-plan-add', type: 1}));
  };

  // 插入临时计划
  insertTempPlan = () => {
    this.props.dispatch(routerRedux.push({pathname: '/query/patrol-plan-add', type: 0}));
  };
  ...
  render() {
    ...
    const columns = [
      {
        title: '计划名称', dataIndex: 'name', key: 'name', width: '20%',
        render(text, record) {
          return <div>
        ...
        title: '巡检员', dataIndex: 'usernames', key: 'usernames', width: '15%',
        render(text, record) {
        ...
        render(text, record) {
          return <div>
            ...
            <div>{record.patrolLayer}{/* : <span style={{color: '#1890ff'}}>20</span>*/}</div>
          </div>;
        }
      }, {
        title: '巡检周期', dataIndex: 'planCycle.name', key: 'planCycle.name', width: '12%',
        render(text, record) {
    ...
    const menu = (data) => (
      <Menu>
        {/*{data.type === 0 ? null : <Menu.Item key="0">*/}
