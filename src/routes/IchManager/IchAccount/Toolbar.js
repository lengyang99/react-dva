...
class Toolbar extends PureComponent {
  ...
  render() {
    const { keyword, eqCode, model, siteValue } = this.state;
    const { regulatorFilter } = this.props;
    return (
      <div className={styles.toolbar__basic}>
        ...
        <div className={styles.toolbar__basic__item}>
          ...
          <Select
            ...
            value={siteValue}
          >
            {(regulatorFilter.pos || []).map(option => <Option key={option.value}>{option.text}</Option>)}
          </Select>
        </div>
        {/*<div className={styles.toolbar__basic__item}>*/}
