...
export default class EmerUserDispatchOrder extends React.Component {
  ...
  handleSendDispatchOrder = () => {
    const {emerEvent, the_emerUser, onCancel} = this.props;
    let fd = new FormData();
    fd.append('alarmId', emerEvent.alarmId);
    fd.append('orderType', 2);
    fd.append('receiver', the_emerUser.username);
  ...
  // 设置定时任务
  handleSetTimeWork = () => {
    const {the_emerUser} = this.props;
    if (userTimer) {
      this.handleRushToIncidentAddr(the_emerUser, 'emerUser', userMark);
    }
  }
