...
export default class FaultList extends React.Component {
  ...
  constructor(props) {
    ...
    // if (pathname === '/equipment/fault-mnmg') {
  ...
  getWorkOrderDetail = (record) => {
    let path = {
      ...
      historyPageName: '/station/CZdefault',
    };
    ...
    const action = (dd) => {
      ...
      return (
        <span style={{float: 'left'}}>
          <span
            style={{marginRight: 20, cursor: 'pointer', color: '#1890ff'}}
            onClick={that.getEventdetail.bind(that, dd)}
          >
          详情
          </span>
          {dd.status === '0' ? dobtn : ''}
    ...
    const field = {
      ...
      search: [
        ...
        <div>
          <Button
            type='primary'
            onClick={this.reportEventClick.bind(this, '5')}
          >故障上报</Button>
        </div>,
      ],
      extra: [
