...
class SearchPanel extends PureComponent {
    handleTypeChange = (parentFunctionKey) => {
      const {searchStandardParams} = this.props;
      const {likeValue, stateValue} = searchStandardParams || {};
      const searchParams = {likeValue, taskCategory: parentFunctionKey, stateValue};
    ...
    onRest = () => {
      ...
      this.props.dispatch({
        ...
        payload: {pageno: 1, pagesize: 10, total: 0},
      });
      this.props.handleOnSearch();
    }
    render() {
      const { catergoryData, searchStandardParams} = this.props;
      const {stateValue, taskCategory, likeValue} = searchStandardParams || {};
      const catergoryOptions = (catergoryData || []).map(item =>
        <Option key={item.functionKey} value={item.functionKey} >{item.functionName}</Option>
      );
