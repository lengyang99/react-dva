...
import WithFunctionGroup from './WithFunctionGroup';
...
@connect(({ device, login}) => ({
  ...
  rowIndex: device.rowIndex, // 选中行下标
  searchPlanParams: device.searchPlanParams, // 搜索條件
}))
@WithFunctionGroup('prev_maintain')
export default class DeviceMaintenance extends Component {
  modal = null;
  // 是否显示导入计划弹框
  showModal = () => {
    if (this.modal) {
