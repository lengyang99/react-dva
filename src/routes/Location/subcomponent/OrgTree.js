...
const loop = list => {
  return !Array.isArray(list) ? null : list.map(ele => {
    if (typeof ele.children === 'undefined' && Array.isArray(ele.children)) {
      return <TreeNode title={ele.text} key={ele.attributes.code} value={ele.attributes.code} />;
    } else {
      return (
        <TreeNode title={ele.text} key={ele.attributes.code.toString()} value={ele.attributes.code.toString()}>
          {loop(ele.children)}
        </TreeNode>
      );
    }
