...
import { getPersonTree } from '../../../../services/patrolTrace';
...
class showPatrolPerson extends Component {
  ...
  componentWillUnmount() {
    ...
    if (this.map.getMapObj() && this.map.getMapObj().infoWindow) {
  ...
  getPersonTrees = (loadType) => {
    getPersonTree({ userid: this.props.user.gid, loadType: loadType }).then((res) => {
      if (!res.success) {
      ...
      let personTree = [];
      let personList = [];
      this.queryPersonCall(res.data, personTree, personList);
  ...
  onCheckPerson = (value, target) => {
    ...
    for (let i = 0; i < value.length; i++) {
      for (let j = 0; j < this.state.personList.length; j++) {
        if (value[i] === this.state.personList[j].id) {
          showPointData.push(this.state.personList[j]);
        }
      }
    }
  ...
  showTrace = (e) => {
    ...
    for (let i = 0; i < this.state.defaultPerson.length; i++) {
      for (let j = 0; j < this.state.personList.length; j++) {
        if (this.state.defaultPerson[i] === this.state.personList[j].id) {
          let tmpperson = Object.assign({}, this.state.personList[j]);
  ...
  showOnlinePerson = (type, e) => {
    ...
    this.showTypeList[type] = checked;

    let tmpIdList = [];
    let tmpDataList = [];
    let tmpList = this.state.personList;
  ...
  showPersonTrace = (value, e) => {
    ...
    for (let i = 0; i < this.state.personList.length; i++) {
      if (value === this.state.personList[i].id) {
        showTracePerson = Object.assign({}, this.state.personList[i]);
  ...
  selectChangePerson = (e, v) => {
    ...
    let showPointData = [];
    for (let j = 0; j < this.state.personList.length; j++) {
      if (value === this.state.personList[j].id) {
        showPointData.push(this.state.personList[j]);
      }
    }
  ...
  */
  ...
  render() {
    ...
    for (let i = 0; i < this.state.personList.length; i++) {
      personList.push(
        <Option
          key={this.state.personList[i].id}
          value={this.state.personList[i].name}
          data-value={this.state.personList[i].id}
        >
          {this.state.personList[i].name}
        </Option>
      );
    }

    let allPersonNum = this.state.personList.length;
    return (
      <div>
        <Dialog
          title="人员位置"
          ...
          <div style={{ height: 400 }}>
            ...
            <div className={styles.show_dialog_person_tree_div}>
              <div className={styles.show_dialog_person_tree_total}>
                在线人数： {this.state.onlineNum} / {allPersonNum}
              </div>
              <hr className={styles.show_patrol_person_hr} />
              <div className={styles.show_dialog_person_tree_data_tree}>
                {
                  this.state.personTreeData.length > 0 ?
                    <Tree
                      checkable
                      defaultExpandedKeys={[this.state.personTreeData[0].value]}
                    ...
                    >
                      {this.loop(this.state.personTreeData)}
                    </Tree> : null
                }
              </div>
            </div>
